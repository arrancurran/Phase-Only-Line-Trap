...
class HologramApp:
    ...
    def _build_ui(self):
        ...
        for entry in (l_entry, angle_entry, lox_entry, loy_entry, sox_entry, soy_entry):
    ...
    def _nudge_entry(self, entry, direction):
        ...
        for key, var in self.vars.items():
            if var is entry.cget("textvariable"):
                name = key
                break

        # Fallback: if we cannot resolve the name, do nothing
        if name is None:
            # Try to interpret the current text directly
        ...
        # Choose step sizes (you can tweak these)
        if name == "angle":
            step = 1.0
        else:
            step = 1.0
